...
class MeasurementPanel extends FXMLContainer {
    ...
    private void setContent(Segment segment, TimeSliceSetting timeSliceSetting) {
        ...
        MeasurementTableFactory factory = new MeasurementTableFactory(List.of(segment), timeSliceSetting);
        for (Measurement<?> m : segment.getMeasurements()) {
            tabPane.getTabs().add(new Tab(m.getMeasurementType().getName(), factory.forMeasurementType(m.getMeasurementType())));
        }
    }
